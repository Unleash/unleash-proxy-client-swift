...
@available(iOS 13, *)
func setup(
    dataGenerator: @escaping () -> [String: Toggle],
    session: PollerSession = MockPollerSession(),
    ...
    let unleash = UnleashProxyClientSwift
        .UnleashClient(
...
func setup(
    ...
    let unleash = UnleashProxyClientSwift
        .UnleashClient(
            unleashUrl: "https://app.unleash-hosted.com/hosted/api/proxy",
            clientKey: "dss22d", refreshInterval: 15,
            appName: "test",
            environment: "dev",
            poller: poller,
            metrics: metrics
    ...
    try await unleash.start(printToConsole: printToConsole)
    return unleash
}
